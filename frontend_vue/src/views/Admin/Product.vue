  ...
  <main>
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
      <div class="w-full mb-1">
        <Breadcrumb header="All products" breadCrumb="Product" link="products" />
        <div class="sm:flex">
...
const columns = ["name", "description", "category", "price", "sale_price", "updated_at", "actions"];
const data = ref([]);
...
const formDataInputs = [
  {
    ...
    label: "Image",
    type: "image",
    error_label: "Need to require image.",
    error: "required",
  },
  {
    id: "name",
  ...
  },
  {
    ...
    label: "Description",
    type: "text",
    error_label: "Need to require description.",
    error: "required",
  },
  {
    id: "category_id",
    label: "Category",
    type: "select",
    error_label: "Need to require category.",
    error: "required",
  },
  {
    id: "price",
  ...
  },
  {
    ...
    label: "Sale Price",
    type: "number",
    error_label: "Need to require sale price.",
    error: "required",
  },
  {
    id: "slug",
...
onMounted(async () => {
  try {
    ...
    dataSelect.value = categories.data;
  } catch (err) {
    toast.add({
      severity: "error",
      summary: "Error",
...
const callback = async (dataCallBack) => {
  if (dataPopUpItem.value.isCreate) {
    try {
      const res = await ProductsService.createProduct(dataCallBack);
    ...
    try {
      const res = await ProductsService.updateProduct(dataCallBack);
      console.log(dataCallBack);
      const index = data.value.findIndex((item) => item.id === dataCallBack.id);
...
const callbackChoose = async (choose) => {
  if (choose.isDelete) {
    try {
      await ProductsService.deleteProduct(dataPopUpDelete.value.dataItem.id);
      const index = data.value.findIndex((item) => item.id === dataPopUpDelete.value.dataItem.id);
