  ...
  <main>
    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
      <div class="w-full mb-1">
        <Breadcrumb header="All categories" breadCrumb="Categories" link="categories" />
        <div class="sm:flex">
...
const columns = ["name", "slug", "actions"];
const data = ref([]);
...
onMounted(async () => {
  try {
    const categories = await CategoriesService.getCategories(page.value);
...
const callback = async (dataCallBack) => {
  try {
    if (dataPopUpItem.value.isCreate) {
      const category = await CategoriesService.createCategory(dataCallBack);
      ...
      const category = await CategoriesService.updateCategory(dataCallBack);
...
const callbackChoose = (choose) => {
  if (choose) {
    try {
      CategoriesService.deleteCategory(dataPopUpDelete.value.dataItem.id);
