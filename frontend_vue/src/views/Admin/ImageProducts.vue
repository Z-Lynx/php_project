  ...
  <main>
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
      <div class="w-full mb-1">
        <Breadcrumb header="All image product" breadCrumb="Image Products" link="image-products" />
        <div class="sm:flex">
...
import ImageProductsService from "../../services/imageProducts.service";
...
const columns = ["image", "product_id", "action"];
const data = ref([]);
...
const formDataInputs = [
  ...
  },
  {
    ...
    label: "Product Id",
    type: "select",
    error_label: "Need to require product id.",
    error: "required",
  },
];
...
onMounted(async () => {
  try {
    const categories = await ImageProductsService.getImageProducts(page.value);
    const products = await ProductsServices.getProducts();
    dataSelect.value = products.data;
  ...
  try {
    if (dataPopUpItem.value.isCreate) {
      ...
      toast.add({
        severity: "success",
        summary: "Success",
        detail: "Create image product success",
        life: 1500,
      });
    } else {
      ...
      toast.add({
        severity: "success",
        summary: "Success",
        detail: "Update image product success",
        life: 1500,
      });
    }
  } catch (err) {
...
const callbackChoose = (choose) => {
  if (choose) {
    try {
      ImageProductsService.deleteImageProduct(dataPopUpDelete.value.dataItem.id);
      data.value = data.value.filter((item) => item.id !== dataPopUpDelete.value.dataItem.id);
      toast.add({
        severity: "success",
        summary: "Success",
        detail: "Delete image product success",
        life: 1500,
      });
    } catch (err) {
      toast.add({
